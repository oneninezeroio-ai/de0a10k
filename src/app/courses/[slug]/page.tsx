...
import type { Module, Lesson, UserProgress, WorkbookResponse } from '@/lib/types'
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: (data as { title: string } | null)?.title ?? 'Módulo' }
}
...
export default async function CoursePage({ params }: PageProps) {
  ...
  const { data: moduleRaw } = await supabase.from('modules').select('*').eq('slug', params.slug).single()
  const module = moduleRaw as Module | null
  if (!module) notFound()
  ...
  const { data: workbookRaw } = await supabase.from('workbook_responses').select('is_complete,updated_at').eq('user_id', user.id).eq('module_id', module.id).single()
  const workbookData = workbookRaw as Pick<WorkbookResponse, 'is_complete' | 'updated_at'> | null
  ...
  const completedCount = lessons.filter((l: Lesson) => progressMap.get(l.id)?.status === 'completed').length
  const pct = lessons.length > 0 ? Math.round((completedCount / lessons.length) * 100) : 0
  const totalMins = lessons.reduce((sum: number, l: Lesson) => sum + (l.duration_mins ?? 0), 0)
    ...
    <div style={{ padding: 'clamp(24px,4vw,48px)', maxWidth: '800px', paddingBottom: '80px' }}>
      ...
      <div className="animate-up" style={{ marginBottom: '40px', paddingBottom: '40px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '20px', fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-4)' }}>
          ...
          {lessons.some((l: Lesson) => l.has_quiz) && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
